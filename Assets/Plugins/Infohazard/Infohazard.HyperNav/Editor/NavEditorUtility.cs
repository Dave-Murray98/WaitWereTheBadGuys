...
namespace Infohazard.HyperNav.Editor {
    ...
    public static class NavEditorUtility {
        ...
        public static void ExportPreviewMesh(Mesh mesh) {
            ...
            for (int i = 0; i < smCount; i++) {
                ...
                for (int j = 0; j < triCount; j++) {
                    ...
                    builder.Append($"f {tris[t + 2] + 1} {tris[t + 1] + 1} {tris[t + 0] + 1}").Append(Environment.NewLine);
                }
            }
        ...
        /// </summary>
        [MenuItem("Tools/Infohazard/Create/Nav Surface", priority = 1)]
        public static void CreateSurface() {
            CoreEditorUtility.CreateGameObjectInSceneWithComponent<NavSurface>("NavSurface");
        }

        /// <summary>
        /// Menu item to create a new NavVolume + NavSurface object.
        /// </summary>
        [MenuItem("Tools/Infohazard/Create/Nav Volume + Surface", priority = 1)]
        public static void CreateVolumeAndSurface() {
            NavVolume volume = CoreEditorUtility.CreateGameObjectInSceneWithComponent<NavVolume>("NavVolume/Surface");
            NavSurface surface = volume.gameObject.AddComponent<NavSurface>();
            Undo.RegisterCreatedObjectUndo(surface.gameObject, "Create NavVolume/Surface");
        }
        ...
        /// </summary>
        [MenuItem("Tools/Infohazard/Create/Manual Nav Link", priority = 1)]
        public static void CreateManualLink() {
            CoreEditorUtility.CreateGameObjectInSceneWithComponent<ManualNavLink>("ManualNavLink");
        }
    }
}
