...
namespace Infohazard.HyperNav {
    ...
    public struct SplinePath : IDisposable {
        ...
        /// <param name="start">Waypoint index of the start of the path.</param>
        /// <param name="end">Waypoint index of the end of the path.</param>
        ...
        public SplinePath(NavPath path, int start, int end, float tangentScale, int sampleCount, bool raycastTangents) :
            this() {
            Initialize(path, start, end, tangentScale, sampleCount, raycastTangents);
        }
        ...
        /// <param name="sampleCount">How many samples to take per segment of the spline when mapping the distance.</param>
        /// <param name="raycastTangents">Whether to shorten tangents by raycasting against NavVolume blocking triangles.</param>
        public void Initialize(NavPath path, int start, int end, float tangentScale, int sampleCount,
                               bool raycastTangents) {

            // Create list of points from the path, removing any that are within a small range.
            CalculateTempPoints(path, start, end);
        ...
        public NavVolume GetVolume(float parameter) {
            if (PointCount == 0) {
                ...
                return NavVolume.Instances.TryGetValue(volumeID, out NavVolume volume) ? volume : null;
            }
        }
        ...
        private static void CalculateTempPoints(NavPath path, int start, int end) {
            float4 vLast = float4.zero;
            TempPoints.Clear();
            int count = end - start + 1;
            for (int i = 0; i < count; i++) {
                int waypointIndex = start + i;
                NavWaypoint waypoint = path.Waypoints[waypointIndex];
                float4 v = new(waypoint.Position, 1);
                long fromVolume = waypoint.AreaID;
                long toVolume = waypoint.AreaID;

                // For external connections, only one point should be added to the spline.
                if (waypoint.Type == NavWaypointType.ExitVolume &&
                    i < count - 1 &&
                    path.Waypoints[waypointIndex + 1].Type == NavWaypointType.EnterVolume) {
                    v = (v + new float4(path.Waypoints[waypointIndex + 1].Position, 1)) / 2.0f;
                    toVolume = path.Waypoints[waypointIndex + 1].AreaID;
                    i++;
                }
        ...
        private void DoTangentRaycastChecks() {
            ...
            NativeArray<NativeRaycastElement> raycasts = new(raycastCount, Allocator.TempJob);
            ...
            NavMultiRaycastJob job = new() {
                Raycasts = raycasts,
                Volumes = NavVolume.NativeDataMap,
            };

            int batchSize = JobUtility.GetMinimumBatchSizeToAvoidUsingAllCores(raycastCount);
            JobHandle handle = job.Schedule(raycastCount, batchSize);
            handle.Complete();
