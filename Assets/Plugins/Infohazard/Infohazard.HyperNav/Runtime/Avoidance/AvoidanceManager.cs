...
namespace Infohazard.HyperNav {
    ...
    public class AvoidanceManager : Singleton<AvoidanceManager> {
        ...
        [HelpBox("How far in the future to look when avoiding collisions. " +
        ...
        [HelpBox("The maximum number of obstacles that can be considered by each agent for avoidance. " +
                 "Generally it should be equal to the max number of obstacles you expect to be within " +
                 "TimeHorizon * IAvoidanceObstacle.MaxSpeed of an agent.")]
        private int _maxObstaclesConsidered = 10;

        public static class PropNames {
            public const string UpdateMode = nameof(_updateMode);
            public const string TimeHorizon = nameof(_timeHorizon);
            public const string MaxObstaclesConsidered = nameof(_maxObstaclesConsidered);
        }
        ...
        private static readonly ProfilerMarker MarkerUpdateAvoidance = new("AvoidanceManager.UpdateAvoidance");

        private static readonly ProfilerMarker MarkerUpdateDataStructures = new("AvoidanceManager.UpdateDataStructures");

        private static readonly ProfilerMarker MarkerObstacleLoop =
            new("AvoidanceManager.UpdateAvoidance Obstacle Loop");

        private static readonly ProfilerMarker MarkerGetInputVelocity =
            new("AvoidanceManager.UpdateAvoidance Get Input Velocity");

        private static readonly ProfilerMarker MarkerRunJob = new("AvoidanceManager.UpdateAvoidance Run Job");

        private static readonly ProfilerMarker MarkerAgentLoop = new("AvoidanceManager.UpdateAvoidance Agent Loop");
        ...
        public virtual void UpdateAvoidance(float deltaTime) {
            try {
                UpdateAvoidanceInternal(deltaTime);
            } catch (Exception e) {
                Debug.LogException(e);
            }
        }

        private void UpdateAvoidanceInternal(float deltaTime) {
            ...
            using (MarkerObstacleLoop.Auto()) {
                for (int i = 0; i < Avoidance.AllObstacles.Count; i++) {
                    ...
                    NativeAvoidanceObstacleData obstacleData = new() {
                        Avoidance = 0,
                        Padding = 0,
        ...
        private void UpdateDataStructures() {
            ...
            if (_allocatedObstacleCount < obstacleCount) {
                ...
                _allocatedObstacleCount = Mathf.RoundToInt(obstacleCount * 2);
            ...
            if (_allocatedAgentCount < agentCount) {
                ...
                _allocatedAgentCount = Mathf.RoundToInt(agentCount * 2);
            ...
            if (_allocatedPlaneCount < planeCount) {
                ...
                _allocatedPlaneCount = Mathf.RoundToInt(planeCount * 2);
