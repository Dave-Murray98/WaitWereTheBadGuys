...
namespace Infohazard.HyperNav.Jobs.Baking.Volume {
    [BurstCompile]
    public struct GenerateVolumeSphereQueriesJob : IJobParallelFor {
        [NativeDisableParallelForRestriction]
        public NativeArray<OverlapSphereCommand> Queries;
        ...
        void CreateQuery(int queryIndex, Vector3 worldPoint) {
            if (UsePhysicsScene) {
                Queries[queryIndex] =
                    new OverlapSphereCommand(PhysicsScene, worldPoint, QueryRadius, new QueryParameters {
                        hitTriggers = QueryTriggerInteraction.Ignore,
                        layerMask = QueryLayerMask,
                    });
            } else {
                Queries[queryIndex] = new OverlapSphereCommand(worldPoint, QueryRadius, new QueryParameters {
                    hitTriggers = QueryTriggerInteraction.Ignore,
                    layerMask = QueryLayerMask,
                });
            }
