...
namespace Infohazard.HyperNav.Jobs.Utility {
    ...
    public struct UnsafeHeap<T> : IDisposable where T : unmanaged, IEquatable<T> {
        private UnsafeList<Element> _items;
        ...
        public UnsafeHeap(int initialCapacity, Allocator allocator) {
            _items = new UnsafeList<Element>(initialCapacity, allocator);
        }
        ...
        /// <exception cref="ArgumentException">Thrown when the item is not in the heap.</exception>
        public void Update(T item, float newPriority, bool replace = false, T replaceWith = default) {
            bool result = TryUpdate(item, newPriority, replace, replaceWith);
            if (!result) {
        ...
        /// <param name="replace">Whether to replace the item as well.</param>
        /// <param name="replaceWith">What item to replace it with.</param>
        /// <returns>Whether the item was successfully updated.</returns>
        public bool TryUpdate(T item, float newPriority, bool replace = false, T replaceWith = default) {
        ...
        public bool TryPeek(out T value) {
            return TryPeek(out value, out _);
        }
        ...
        /// <returns>Whether the heap had an item to get.</returns>
        public bool TryPeek(out T value, out float priority) {
            if (_items.Length > 0) {
                Element item = _items[0];
                value = item.Item;
        ...
        public bool TryRemove(out T value) {
            return TryRemove(out value, out _);
        }
        ...
        /// <param name="priority">The element's priority.</param>
        /// <returns>Whether the heap had an item to remove.</returns>
        public bool TryRemove(out T value, out float priority) {
            if (!TryPeek(out value, out priority)) return false;
