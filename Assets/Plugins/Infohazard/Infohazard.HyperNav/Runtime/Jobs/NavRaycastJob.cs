...
namespace Infohazard.HyperNav.Jobs {
    ...
    public struct NavRaycastJob : IJob {
        ...
        public void Execute() {
            ...
            bool hit = NativeMathUtility.NavVolumeRaycast(localStart, localEnd, false, Volume, out float t);
    ...
    public struct NavMultiRaycastJob : IJobParallelFor {
        ...
        public void Execute(int index) {
            ...
            if (Volumes.TryGetValue(raycast.VolumeID, out NativeNavVolumeData volume)) {
                float4 localStart = math.mul(volume.InverseTransform, raycast.Start);
                float4 localEnd = math.mul(volume.InverseTransform, raycast.End);
                bool didHit = NativeMathUtility.NavVolumeRaycast(localStart, localEnd, false, volume, out float t);
