...
namespace NWH.NUI
{
    ...
    public class NUIDrawer
    {
        ...
        private readonly GUIStyle   unitStyle   = new();
        private          string     _cachedKey;
        private          DrawerType _drawerType        = DrawerType.Property;
        private          GUIStyle   bgStyle            = new();
        private          Rect       customPositionRect = new(-1, -1, -1, -1);

        // Cached GUIStyles for performance
        private          GUIStyle   _cachedHeaderLabelStyle;
        private          GUIStyle   _cachedInfoBoxStyle;
        private          GUIStyle   _cachedTitleStyle;
        private          GUIStyle   _cachedToolbarButtonStyle;
        ...
        public void BeginEditor(SerializedObject serializedObject)
        {
            ...
            // Note: Background uses cached height from previous frame to avoid measuring content twice.
            // This is acceptable in immediate mode GUI as content changes are typically gradual.
        ...
        public void BeginProperty(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Store the current GUI.enabled state so it can be restored in EndProperty
            EditorCache.SetGuiWasEnabledCacheValue(GenerateKey(), GUI.enabled);

            InitializeGUIElements();

            // Note: Background uses cached height from previous frame to avoid measuring content twice.
            // This is acceptable in immediate mode GUI as content changes are typically gradual.
            Rect totalRect = new(positionRect.x, positionRect.y, positionRect.width, GetHeight());
            DrawBackgroundRect(new Rect(positionRect.x, positionRect.y, positionRect.width, GetHeight()));
        ...
        public void EmbeddedObjectEditor<T>(Object obj, Rect rect, float leftMargin = 0) where T : NUIEditor
        {
            ...
            if (nuiEditor != null)
            {
                ...
                GUILayout.Space(
                    -editorHeight); // Negate space that embedded editor has already added to prevent overly-large final
                // editor
        ...
        public virtual void EndProperty()
        {
            ...
            // Restore the original GUI.enabled state that was saved in BeginProperty
            bool originalGuiEnabled = true;
            if (EditorCache.GetGuiWasEnabledCValue(GenerateKey(), ref originalGuiEnabled))
            {
        ...
        public SerializedProperty FloatSlider(string propertyRelativeName, float min = 0, float max = 1,
            string                                   leftLabel  = "",
            string                                   rightLabel = "", bool showNumValue = true)
        {
            ...
            EditorGUI.LabelField(positionRect,
                                 CultureInfo.CurrentCulture.TextInfo.ToTitleCase(
                                     AddSpacesToSentence(propertyRelativeName, true)));

            Rect  sliderRect = new(positionRect);
            ...
            Rect legendRect = new(sliderRect);
            legendRect.y += 4;
            ...
            string alternateLabel = null, float fieldHeight = -1, bool includeChildren = false)
        {
            ...
            if (unit != null)
            {
                ...
                Rect unitRect = new(fieldRect.x + fieldRect.width - unitRectWidth - 5, fieldRect.y,
        ...
        public bool Header(string label, bool drawFoldoutButton = true)
        {
            ...
            Rect backgroundRect = new(positionRect.x, positionRect.y, positionRect.width, 22);
            ...
            Rect labelRect = new(positionRect.x + NUISettings.textMargin + 15f, positionRect.y - 1, maxLabelWidth,
                                 NUISettings.fieldHeight);
            #else
            ...
            {
                _cachedHeaderLabelStyle = new GUIStyle(EditorStyles.label);
                _cachedHeaderLabelStyle.normal.textColor = Color.white;
            }
            EditorGUI.LabelField(labelRect, new GUIContent(label), _cachedHeaderLabelStyle);

            // Draw help link
            Rect helpRect = new(labelRect.x + labelRect.width + 5f, positionRect.y + 3, 16, 16);
            ...
            if (Event.current.type == EventType.MouseUp && helpRect.Contains(Event.current.mousePosition))
            {
                ...
                string className = fullTypeName.Substring(fullTypeName.LastIndexOf('.') + 1);
                Application.OpenURL($"{documentationBaseURL}?q={className}");
            }
            ...
            float                           targetButtonWidth = 80f, float buttonHeight = 18f)
        {
            ...
            if (positionRect.width > 20f)
            {
                ...
                Rect bottomLine = new(initRect.x - 2f, initRect.y + toolbarHeight, initRect.width, 1f);
                ...
                for (int x = 0; x < rowCount; x++)
                {
                    float buttonWidth = fillWidth
                                            ? x == rowCount - 1
                                                  ? rowWidth / lastRowButtons
                                                  : rowWidth / maxButtonsPerRow
                                            : targetButtonWidth;
                    for (int y = 0; y < subTexts[x].Length; y++)
                    {
                        if (_cachedToolbarButtonStyle == null)
                        {
                            _cachedToolbarButtonStyle = new GUIStyle(EditorStyles.miniButtonMid);
                        }
                        _cachedToolbarButtonStyle.fixedHeight = buttonHeight;

                        Color initColor = GUI.color;

                        Rect buttonRect = new(initRect.x + y * buttonWidth, initRect.y, buttonWidth,
                                              rowHeight + 1);
                        ...
                        if (GUI.Button(buttonRect, subTexts[x][y], _cachedToolbarButtonStyle))
                        {
                            tabIndex = counter;
                        }
                        ...
                        if (tabIndex == counter)
                        {
                            Rect highlightRect = new(buttonRect.x - 2f, buttonRect.y + buttonRect.height - 2,
                                                     buttonRect.width,
                                                     2);
                ...
                EditorCache.SetHeightCacheValue(string.Concat(GenerateKey(), name), toolbarHeight);
            }
            else
            {
                EditorCache.GetHeightCacheValue(string.Concat(GenerateKey(), name), ref toolbarHeight);
            }
        ...
        {
            if (_cachedInfoBoxStyle == null)
            {
                // Use EditorStyles.helpBox for accurate height calculation
                _cachedInfoBoxStyle = new GUIStyle(EditorStyles.helpBox);
                _cachedInfoBoxStyle.wordWrap = true;
            }
            ...
            if (positionRect.width > 20f)
            {
                ...
                // Calculate height using the helpbox style for accurate multiline text sizing
                height = _cachedInfoBoxStyle.CalcHeight(content, positionRect.width);

                // Add padding to account for HelpBox icon and margins
                height = Mathf.Max(height, 40f); // Minimum height for helpbox

                Rect infoRect = new(positionRect.x, positionRect.y, positionRect.width, height);
                EditorGUI.HelpBox(infoRect, text, messageType);

                EditorCache.SetHeightCacheValue(string.Concat(GenerateKey(), text.GetHashCode()), height);
            }
            else
            {
                EditorCache.GetHeightCacheValue(string.Concat(GenerateKey(), text.GetHashCode()), ref height);
            }
        ...
        {
            Property(FindProperty(propertyName), drawChildren);
        }
            ...
            Type                           baseType             = null, float elementSpacing = 0)
        {
            ...
            // and crashing the editor if assigned. Null check does not work in this case - accessing targetObject
            // on a disposed SerializedObject throws NullReferenceException instead of returning null.
            bool needsNewList = reorderableList == null;
            if (reorderableList != null && reorderableList.serializedProperty != null)
            {
                // Try to check if the serializedObject is still valid
                try
                {
                    // This will throw if the serializedObject is disposed
                    if (reorderableList.serializedProperty.serializedObject == null ||
                        reorderableList.serializedProperty.serializedObject.targetObject == null)
                    {
                        needsNewList = true;
                    }
                }
            ...
            Rect  listRect   = new(positionRect.x, positionRect.y + 3f, positionRect.width, listHeight);
        ...
        {
            if (_cachedTitleStyle == null)
            {
                _cachedTitleStyle = new GUIStyle(EditorStyles.label);
                _cachedTitleStyle.fontStyle = FontStyle.Bold;
            }
            _cachedTitleStyle.normal.textColor = EditorGUIUtility.isProSkin ? Color.white : Color.black;
            ...
            #if UNITY_2019_3_OR_NEWER
            EditorGUI.LabelField(positionRect, new GUIContent(" " + label), _cachedTitleStyle);
            #else
            EditorGUI.LabelField(new Rect(positionRect.x, positionRect.y + 3f, positionRect.width, positionRect.height),
                                 new GUIContent(" " + label), _cachedTitleStyle);
            #endif
